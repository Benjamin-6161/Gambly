...
# NOTE: This is a copy of fetch_all_matches.py for local testing, with the
# `save_matches` DB step removed. It just prints out whatever matches were
# found instead of persisting them, so you can sanity-check scraping
# without touching resources/db/matches.db.
...
def fetch_matches_test(*args, **kwargs):
    """
    Same scraping logic as fetch_matches() in fetch_all_matches.py, but
    with the save_matches(...) DB write removed - just returns/prints the
    ...
    # NOTE: save_matches(all_matches) intentionally removed for this test script.
